...
public class ProductDBContext extends DBContext {
    ...
    public List<Product> getProductsByCategoryId(int categoryId, int page, int PAGE_SIZE) {
        List<Product> list = new ArrayList<>();
        try {
            String sql = "select * from Products where cid = ? order by id\n"
    ...
    public List<Product> search(String keyword, int page, int PAGE_SIZE, int categoryId) {
        List<Product> list = new ArrayList<>();
        try {
    ...
    public int countProducts(String keyword , int categoryId) {
        List<Product> list = new ArrayList<>();
        try {
            String sql = "SELECT COUNT(*) as totalProducts from Products where name like ? ";
            if (categoryId != 0) {
                sql += " and cid = ? \n";
            }
            PreparedStatement stm = connection.prepareStatement(sql);
    ...
    public int countProducts(int category) {
        List<Product> list = new ArrayList<>();
        try {
            String sql = "SELECT COUNT(*) as totalProducts from Products where cid = ?";
