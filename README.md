...
| 🛍️ **Customer** (Khách hàng) | Đăng ký, đăng nhập, quản lý tài khoản, thêm vào giỏ hàng, đặt hàng |
...
- 🖼️ **View** — `web/*.jsp`: các file template hiển thị giao diện cho người dùng
...
- 🗄️ **Microsoft SQL Server** (bản Developer hoặc Express) — [Tải về](https://www.microsoft.com/en-us/sql-server/sql-server-downloads)
- 🖥️ **SQL Server Management Studio (SSMS)** — Công cụ quản lý database
...
> ⚠️ Nếu cấu hình SQL Server trên máy bạn khác, hãy chỉnh sửa file `src/java/dal/DBContext.java` cho phù hợp.
...
- ✏️ **Chỉnh sửa tài khoản** — Cập nhật địa chỉ, số điện thoại, email và các thông tin cá nhân khác.
...
- 🗑️ **Xoá sản phẩm** — Xoá từng item khỏi giỏ hàng.
...
| 🗑️ **Xoá** | `DeleteCartController` | Xoá entry khỏi `LinkedHashMap` |
...
> ⚠️ **Quan trọng:** Sử dụng **Gmail App Password** (không phải mật khẩu Gmail thông thường). Bật 2FA trên tài khoản Google, sau đó tạo App Password tại Google Account → Security → App Passwords.
...
### 🗄️ Kết Nối Database (`src/java/dal/DBContext.java`)
...
*🛍️ Built with Java Servlet/JSP · Bootstrap 4 · SQL Server · Apache Tomcat*
